...
gatherInputFiles <- function(
  ...
  for(fn in inputFileNames) {
    ...
    while(nrow(dat) > 0) {
      ...
      # separate head and another part of data
      nr1 <- which(dat$V6 == dat$V7)[1]
      dat0 <- dat[1:nr1, ]
      dat  <- dat[- (1:nr1), ]

      # find validation pattern for head of data
      valPattern <- names(val)[sapply(val, valFind, x = dat0$V8)]
      ...
      if(length(valPattern) == 0) {
        ...
        errorRow <- as.integer(rownames(dat)[1]) - 1
        cat(paste('No validation pattern was found for row', errorRow))
        cat('\n')
        error2 <- T

      } else if (length(valPattern) > 1) {

        errorRow <- as.integer(rownames(dat)[1]) - 1
        cat(paste('Multiple validation pattern was hit for row', errorRow))
        cat('\n')
        error2 <- T
